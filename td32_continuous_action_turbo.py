...
class Args:
    ...
    wandb_project_name: str = "gradient_critic_td3"
...
"""
    ...
    qf1 = QNetwork(envs,num_params).to(device)
    qf2 = QNetwork(envs,num_params).to(device)
    target_actor = Actor(envs).to(device)
    target_actor.load_state_dict(actor.state_dict())
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()), lr=args.learning_rate)
    gamma_optimizer = optim.Adam([qf1.fc4.weight, qf1.fc4.bias]+[qf2.fc4.weight, qf2.fc4.bias], lr=args.learning_rate)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                def compute_actor_loss(params,buffers,data):

                    obs = data.next_observations.unsqueeze(0) ###MAKE SURE TO USE NEXT OBS
                    action = functional_call(actor,(params,buffers),(obs,))
                    actor_loss = -(qf1(obs, action)+qf2(obs, action)).mean()
                ...
                gamma1_next_target = qf1.forward_gamma(data.next_observations, next_state_actions)
                gamma2_next_target = qf2.forward_gamma(data.next_observations, next_state_actions)
            ...
            if global_step % args.policy_frequency == 0:
                ...
                actor_loss = -(qf1(data.observations,pi)+qf2(data.observations,pi)).mean()
